...
defineComponent({
  ...
  render(ctx) {
    ...
    return html`
      <div class="border border-[var(--wp-border)] rounded-lg p-4 bg-[var(--wp-surface)] space-y-3 text-[var(--wp-fg)]">
        <h3 class="text-sm font-semibold text-[var(--wp-muted)]">${title}</h3>
        ...
        ${ctx.state.lastEvent
          ? html`<p class="text-xs text-[var(--wp-subtle)]">${ctx.state.lastEvent}</p>`
          : ''}
      </div>
    `
  },
